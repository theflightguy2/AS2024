...
def ChangeSettings():
  ...
  if Answer == 'Y':
    ...
    SimulationTime = input("Simulation run time: ")
    valid = False
    while not valid:
      if not SimulationTime.isnumeric():
          print("NON NUMERIC INPUT! NEVER AGAIN.")
          SimulationTime = input("Simulation run time: ")
          continue
      else:
        SimulationTime = int(SimulationTime)
        if SimulationTime > MAX_TIME or SimulationTime < 1:
          print(f"Maximum simulation time is {MAX_TIME} time units")
          SimulationTime = input("Simulation run time: ") 
          continue
        else:
          print(f"Maximum number of tills is {MAX_TILLS}")
          NoOfTills = input("Number of tills in use: ")
          if not NoOfTills.isnumeric():
            print("NON NUMERIC INPUT! NEVER AGAIN.")
            SimulationTime = input("Simulation run time: ")
            continue
          else:
            NoOfTills = int(NoOfTills)
            if NoOfTills > MAX_TILLS or NoOfTills < 1:
              print(f"Maximum number of tills is {MAX_TILLS}")
              NoOfTills = input("Number of tills in use: ")
              continue
            else:
